...
function mountPanel() {
  ...
  panelStopBtn?.addEventListener('click', () => {
...
function postToPanel(_msg: any) {
  // No-op: the panel now renders in-page (shadow DOM) instead of an iframe.
}

function updatePanelState() {
  if (!panelPauseBtn || !panelStopBtn) return
  panelPauseBtn.disabled = !picking
  panelPauseBtn.setAttribute('aria-pressed', inspectPaused ? 'true' : 'false')
  panelPauseBtn.title = inspectPaused ? 'Resume inspector' : 'Pause inspector'
  panelStopBtn.title = picking ? 'Stop inspector' : 'Start inspector'

  panelPauseIcon?.toggleAttribute('hidden', inspectPaused)
  panelPlayIcon?.toggleAttribute('hidden', !inspectPaused)
  panelStopIcon?.toggleAttribute('hidden', !picking)
  panelStartIcon?.toggleAttribute('hidden', picking)
}
...
function isInsideQwikCSSUI(el: Element) {
  ...
  if (root instanceof ShadowRoot) {
    ...
    if (host instanceof HTMLElement) {
      return (
        host.id === CONTAINER_ID || host.id === TOOLBAR_ID || host.id === CARD_ID
      )
    }
  }
